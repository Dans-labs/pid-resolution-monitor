...
from schemas.schemas import PidMrResolutionEvent, User, PidResolutionRecord
from database.models import MonitorRecord, PIDMREvent
...
async def create_event(event: PidMrResolutionEvent, user: Annotated[User, Depends(get_current_enabled_user)],
    ...
    try:
        ...
        logger.info(f"PIDMR event saved: {db_event.pid_endpoint} by user {user.username}") #TODO: determine what to log.
        ...
        return PidMrResolutionEvent(id=db_event.id, time_stamp=db_event.time_stamp, pid_id=db_event.pid_id,
                                   ...
                                   pid_endpoint=db_event.pid_endpoint, pid_resolver_status=db_event.pid_resolver_status)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Internal Server Error: {e}")


@router.get("/event/{pidmr_event_id}", response_model=PidMrResolutionEvent, summary="Get registered PIDMR Event by PIDMR Event ID", dependencies=[Depends(get_current_enabled_user)])
def get_pid_resolution_record(pidmr_event_id: int, db: Session = Depends(get_db)):
    ...
    if not record:
        raise HTTPException(status_code=404, detail=f"PIDMR Event record not found for PIDMR Event ID: {pidmr_event_id}")
    return record
