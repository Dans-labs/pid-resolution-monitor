...
def get_uptime_by_actor_inst_id(input_data: UptimeMonitorsRequest, user: Annotated[User, Depends(get_current_enabled_user)]):
...
@router.put("/uptimerobot/update", status_code=201, summary="Re-creates pid_graph identifier to UptimeRobot identifier mapping.",
            description="i.e: PID Graph ID (pid_graph:E2045F7A) -> UptimeRobot Monitor ID (797637034).", dependencies=[Depends(get_current_enabled_user)])
def update_uptime_monitors():
    # to get user from injected dependency:
