...
@router.post("/token", summary="Get a JWT token to authenticate.", response_model=Token, include_in_schema=True)
async def login_for_access_token(form_data: Annotated[OAuth2PasswordRequestForm, Depends()],
    ...
    access_token_expires = timedelta(days=settings.JWT_TOKEN_EXPIRE_DAYS)
    access_token = create_access_token(data={"sub": user.username}, expires_delta=access_token_expires)
    return Token(access_token=access_token, token_type="bearer")


@router.get("/users/whoami/", response_model=User, include_in_schema=False)
